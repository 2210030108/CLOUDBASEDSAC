...
// })
const hashFileData = (filePath) => {
    try {
        const fileData = fs.readFileSync(filePath); // Read the file data
        const hashedData = crypto.createHash("sha256").update(fileData).digest("hex"); // Hash the data
        return hashedData;
    } catch (error) {
        throw new ApiError(500, "Error hashing file data");
    }
};
...
const addStudentDetails = asyncHandler(async (req, res) => {
    ...
    // Hash the file data
    const AadhaarHash = hashFileData(AadhaarLocalPath);
    const SecondaryHash = hashFileData(SecondaryLocalPath);
    const HigherHash = hashFileData(HigherLocalPath);

    // Save the hashed data in the database
    const studentdetails = await studentdocs.create({
        Phone,
        Address,
        Highesteducation,
