...
const addTeacherDetails = asyncHandler(async(req,res)=>{
    ...
    let Aadhaar, Secondary, Higher, UG, PG;
    try {
        // Upload all documents to Cloudinary
        [Aadhaar, Secondary, Higher, UG, PG] = await Promise.all([
        ...
        if (!Aadhaar?.url || !Secondary?.url || !Higher?.url || !UG?.url || !PG?.url) {
            throw new ApiError(500, "One or more document uploads failed");
        }
        ...
        if(!theTeacher){
            throw new ApiError(400,"Failed to update teacher details");
        }
        ...
        if (Aadhaar?.public_id) cleanupPromises.push(cloudinary.uploader.destroy(Aadhaar.public_id));
        if (Secondary?.public_id) cleanupPromises.push(cloudinary.uploader.destroy(Secondary.public_id));
        if (Higher?.public_id) cleanupPromises.push(cloudinary.uploader.destroy(Higher.public_id));
        if (UG?.public_id) cleanupPromises.push(cloudinary.uploader.destroy(UG.public_id));
        if (PG?.public_id) cleanupPromises.push(cloudinary.uploader.destroy(PG.public_id));
        
        await Promise.all(cleanupPromises).catch(cleanupError => {
